...
class APIServer(WebUI):
    ...
    def _generate_record_params(self, data: dict, merge=True, **kwargs):
        root, params, logger = self.record.run(
            self.parameter, blank=data["source"], **kwargs
        )
        if not merge:
    ...
    def run_server(self, app):
        ...
        def detail():
            ...
            root, params, logger = self._generate_record_params(data, merge=False)
            with logger(root, console=self.console, **params) as record:
                return {
                    "data": (d := self.input_links_acquisition(record=record, **data)),
                    "message": "success" if d else "failure",
                }
        ...
        def live():
            ...
            return {
                "data": (
                    d := (
                        live_data
                        if request.json.get("source", False)
                        else self.extractor.run(live_data, None, "live")
                    )
                ),
                "message": "success" if d else "failure",
            }
        ...
        def comment():
            ...
            if not ids:
                ...
                return {
                    "data": None,
                    "message": "目前项目暂不支持采集 TikTok 作品评论数据！",
                }
            id_ = ids[0]
            name = f"作品{id_}_评论数据"
            ...
            with logger(root, name=name, console=self.console, **params) as record:
                if result := Comment(
                        ...
                        pages=request.json.get("pages"),
                        cookie=request.json.get("cookie"),
                ).run(self.extractor, record, **data):
        ...
        def user():
            ...
            return {
                "data": (
                    d := self._deal_user_data(**params, data=[i for i in users if i])
                ),
                "message": "success" if d else "failure",
            }
        ...
        def hot():
            time_, data = self._deal_hot_data(source=request.json.get("source", False))
            return {
                "time": time_,
                "data": data,
