...
* 批量下载账号作品或合集作品时，如果该账号昵称发生变化，程序会自动更新已下载作品文件名称中的昵称信息
* 批量下载账号作品或合集作品时，不要打开或访问相关的文件夹和文件，否则文件拒绝访问将会导致预期之外的异常
...
|                       name                        |                   str                   | 文件保存时的命名规则，值之间使用空格分隔<br>默认值: 发布时间-账号标识-描述<br>`id`: 唯一值, `desc`: 描述, `create_time`: 发布时间, `nickname`: 账号昵称, `mark`: 账号标识, `uid`: 账号UID |
