...
- **博主整号下载**(粘主页): `Volume/UID{数字}_{博主名称}/`,每个博主一个扁平文件夹(无 `_发布作品` 后缀)。
- **我的收藏**: `Volume/{抖音分类}/UID{数字}_{博主名称}/` 两级 —— 分类取作品的 `video_tag` 大类(财经/科技/旅游/随拍…),没有标签的进 `未分类/`。
- 同一视频既被整号下载又在收藏里时,归类优先(进 `分类/博主` 夹)。
