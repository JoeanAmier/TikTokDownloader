...
- **关注的博主面板**(窗口上部) —— 「⬇ 拉取关注列表」纯 Cookie 拉你的抖音**关注列表**并**自动给每个关注定主类**(各拉一页取 video_tag 大类,每10个存盘、可⏹停止续);按分类横排(每行8个等宽、随窗口拉伸显示全名),顺序 财经→科技→按博主数→未分类垫底;勾选 → 「🔄 更新选中」增量拉新作品;登记表 `Volume/authors.json` 自动维护;每个博主名后显示**作品数**;「**下载范围**」下拉(全部/最新100/200/500 或自定义数字)→ 大博主只下最新 N 个(小博主自然全下),只拉需要的页
