...
<h2>扫码登陆写入 Cookie</h2>
<p>程序自动获取抖音登录二维码，并在终端输出二维码，程序还会尝试使用系统默认图片浏览器打开二维码图片，使用者通过抖音 APP 扫码并登录账号，操作后关闭二维码图片窗口（如果成功打开二维码图片窗口），程序会自动检查登录结果并将登陆后的 Cookie 写入配置文件。</p>
<h2>终端命令行模式</h2>
<p>功能最全面的模式，支持全部功能。</p>
<h3>批量下载账号作品</h3>
<ol>
...
<li>程序询问是否下载直播时，输入直播清晰度对应索引即可下载，例如：下载最高清晰度输入 <code>0</code></li>
